    ...
    private async calculateBuyXGetYDiscount(
        ...
        for (const product of products) {
            ...
            if (discount.applicableCategories?.length > 0) {
                const hasApplicableCategory = product.categoryIds.some((categoryId: any) =>
                    discount.applicableCategories.some(
                        (applicableCatId: any) => applicableCatId.toString() === categoryId.toString()
                    )
                );
        ...
        for (const item of eligibleItems) {
            const cheapestVariantPrice = Math.min(...item.product.variants.map((v: any) => v.price));
            
            for (let i = 0; i < item.quantity; i++) {
                unitPrices.push(cheapestVariantPrice);
            }
        }

        const totalEligibleItems = unitPrices.length;
        
        const itemsPerGroup = discount.buyX;
        const freeItemsPerGroup = discount.getY;
